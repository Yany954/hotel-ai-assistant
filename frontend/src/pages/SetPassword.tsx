  ...
  return (
    <div className="flex items-center justify-center h-screen bg-slate-50">
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm text-center max-w-sm w-full">
        ...
        {status === "checking" && <p className="text-sm text-slate-500 mt-4">Checking your invite link...</p>}

        {status === "invalid" && (
          <>
            <p className="text-sm text-rose-600 mt-4 mb-2">This link is invalid or has expired.</p>
            <p className="text-sm text-slate-500">Ask an admin to send you a new invite from the Users panel.</p>
          </>
        )}

        {status === "valid" && (
          <form onSubmit={handleSubmit} className="mt-4 text-left">
            <p className="text-sm text-slate-500 mb-4 text-center">Setting a password for <strong>{email}</strong></p>
        ...
        {status === "done" && (
          <div className="mt-4">
            ...
            <p className="text-sm text-slate-600">Password set! Redirecting you to sign in...</p>
          </div>
        )}
      </div>
    </div>
