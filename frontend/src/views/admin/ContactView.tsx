  ...
  return (
    <>
      ...
      <div className="block md:hidden space-y-3">
        {filtered.map((c) => (
          <div key={c.id} className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-violet-50 dark:bg-violet-500/15 flex items-center justify-center text-violet-600 dark:text-violet-400 shrink-0">
                  <Building2 size={16} />
                </div>
                <div>
                  <div className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{c.organizationName}</div>
                  {c.accountNumber && <div className="text-xs text-slate-400 dark:text-slate-500">Account: {c.accountNumber}</div>}
                </div>
              </div>

              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => openEdit(c)} className="p-2 text-slate-500 hover:text-violet-600 hover:bg-violet-50 dark:text-slate-400 dark:hover:text-violet-400 dark:hover:bg-violet-500/10 rounded-lg">
                  <Pencil size={16} />
                </button>
                <button onClick={() => remove(c)} className="p-2 text-slate-500 hover:text-rose-600 hover:bg-rose-50 dark:text-slate-400 dark:hover:text-rose-400 dark:hover:bg-rose-500/10 rounded-lg">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            ...
            <div className="text-xs space-y-1 bg-slate-50 dark:bg-slate-800 p-2.5 rounded-lg">
              {c.phoneLines.map((p, i) => (
                <div key={i} className="text-slate-600 dark:text-slate-300 flex justify-between gap-2">
                  <span className="text-slate-400 dark:text-slate-500 capitalize">{p.purpose || "principal"}:</span>
      ...
      <div className="hidden md:block bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.id} className="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50/60 dark:hover:bg-slate-800/40">
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="font-medium text-slate-800 dark:text-slate-100">{c.organizationName}</div>
                      {c.accountNumber && <div className="text-xs text-slate-400 dark:text-slate-500">Account: {c.accountNumber}</div>}
                    </div>
                  </div>
                </td>
                  ...
                  <div className="space-y-0.5">
                    {c.phoneLines.slice(0, 2).map((p, i) => (
                      <div key={i} className="text-slate-600 dark:text-slate-300">
                        <span className="text-slate-400 dark:text-slate-500">{p.purpose || "principal"}:</span> {p.phoneNumber || "—"}
                      </div>
                    ))}
                    {c.phoneLines.length > 2 && (
                      <div className="text-xs text-violet-600 dark:text-violet-400">+{c.phoneLines.length - 2} more</div>
                    )}
                  </div>
                </td>
                <td className="px-5 py-4 text-right whitespace-nowrap">
                  <div className="flex justify-end gap-1">
                    <button onClick={() => openEdit(c)} className="p-2 text-slate-400 hover:text-violet-600 hover:bg-violet-50 dark:text-slate-500 dark:hover:text-violet-400 dark:hover:bg-violet-500/10 rounded-lg">
                      <Pencil size={15} />
                    </button>
                    <button onClick={() => remove(c)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:text-slate-500 dark:hover:text-rose-400 dark:hover:bg-rose-500/10 rounded-lg">
                      <Trash2 size={15} />
                    </button>
                  </div>
                </td>
      ...
      {showModal && editing && (
        <Modal title={editing.id ? "Edit Contact" : "New Contact"} onClose={() => setShowModal(false)} wide>
          ...
          <div className="mb-2 flex items-center justify-between mt-2">
            <span className="block text-xs font-medium text-slate-500 dark:text-slate-400">Phone Lines</span>
            <button onClick={addPhoneLine} className="text-xs text-violet-600 dark:text-violet-400 font-medium flex items-center gap-1">
              <Plus size={13} /> Add Line
            </button>
          </div>
          <div className="space-y-2 mb-4">
            {editing.phoneLines.map((p, idx) => (
              <div key={idx} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_auto] gap-2 items-center border sm:border-0 border-slate-100 dark:border-slate-800 p-2 sm:p-0 rounded-lg">
                ...
                {editing.phoneLines.length > 1 && (
                  <button onClick={() => removePhoneLine(idx)} className="text-slate-400 hover:text-rose-500 dark:text-slate-500 dark:hover:text-rose-400 self-end sm:self-center p-1">
                    <X size={15} />
                  </button>
                )}
              </div>
